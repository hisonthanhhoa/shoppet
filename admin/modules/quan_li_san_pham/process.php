...
if (isset($_POST['sua_san_pham'])) {
    ...
    $date = getdate();
    // Check if image file is a actual image or fake image
    $url = $target_dir . $date['mday'] . $date['mon'] . $date['year'] . '-' . $date[0] . '.' . $imageFileType;
    ...
    $query = "UPDATE  quan_li_san_pham SET 
            ...
            id_loai_san_pham = $loai_san_pham WHERE id = $id";

    $rs = mysqli_query($conn, $query);
    if (!$rs) {
        echo "<script>alert('Sửa Sản Phẩm Thất Bại, Vui Lòng Thử Lại'); window.location.replace('index.php?c=san_pham&a=index');</script>";
    } else {
